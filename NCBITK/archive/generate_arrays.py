...
def gen_sbatch_array_script(genbank_mirror,
                            array,
                            job_name,
                            mem,
                            time,
    ...
    info_dir, slurm, out, log_file = config.instantiate_path_vars(
        genbank_mirror)
    ...
    with open(sbatch_script, "a") as f:
        ...
        f.write("#SBATCH --output={}\n".format(
            out))  # can I remove this line to avoid getting out files?
        if chunk:
...
def gen_latest_assembly_versions_script(genbank_mirror,
                                        latest_assembly_versions_array):

    info_dir, slurm, out, log_file = instantiate_path_vars(genbank_mirror)
    out = os.path.join(out, "get_latest_%a.out")
    latest_assembly_versions_script = os.path.join(
        slurm, "get_latest_assembly_versions.sbatch")
...
def write_grab_genomes_array(genbank_mirror):
    ...
    latest_assembly_versions = curate.read_latest_assembly_versions(
        genbank_mirror)
    grab_genomes_array = os.path.join(genbank_mirror, ".info", "slurm",
                                      "grab_genomes_array.txt")
    print('Generating {}'.format(grab_genomes_array))
    new_genomes = curate.get_new_genomes(genbank_mirror,
    ...
    with open(grab_genomes_array, "a") as f:
        for group in groups:
            f.write(
                "python /common/contrib/tools/NCBITK/ftp_functions/ftp_functions.py -g {} {}\n".
