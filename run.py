...
def main():
    ...
    parser.add_argument("genbank_mirror",
                        help = "Directory to save fastas",
                        type=str)
    parser.add_argument("-s", "--species",
                        help = 'List of species',
                        nargs='+', default='all')
    parser.add_argument("-p", "--parallel",
                        help = 'Submit jobs in parallel via SLURM pipeline tool.',
                        action="store_true")
    parser.add_argument("-u", "--update",
                        action="store_true")
    parser.add_argument("--use_local_assembly",
                        help = 'Use local copy new assembly_summary.txt and names.dmp',
                        action="store_true")
    parser.add_argument("--status",
                        help = 'Show the current status of your local copy of genbank.',
                        action="store_true")
    ...
    path_vars, assembly_summary, species, genbank_status = setup(genbank_mirror,
                                                 args.species,
    ...
    if args.update:
        ...
        curate.remove_old_genomes(genbank_mirror,
                                  assembly_summary,
                                  local_genomes,
                                  old_genomes,
                                  logger)
